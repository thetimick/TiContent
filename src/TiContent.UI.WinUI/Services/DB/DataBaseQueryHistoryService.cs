...
{
    public Task<List<DataBaseHistoryEntity>> ObtainHistoryAsync(
...
public partial class DataBaseQueryQueryHistoryService : IDataBaseQueryHistoryService
{
    ...
    {
        var items = await db
            .QueryHistoryItems.AsNoTracking()
    ...
    {
        if (
            !db.QueryHistoryItems.Any(entity => entity.Type == type && entity.Query == value.Trim())
        )
        {
            await db.QueryHistoryItems.AddAsync(
                new DataBaseHistoryEntity { Type = type, Query = value.Trim() }
            );
    ...
    {
        if (
            await db.QueryHistoryItems.FirstOrDefaultAsync(entity =>
                entity.Type == type && entity.Query == value.Trim()
            ) is
            { } item
        )
        {
